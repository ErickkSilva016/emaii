...
/**
 * EMAII :: Base de conhecimento do suporte.
 *
 * Este arquivo define QUEM a IA é e O QUE ela sabe sobre o EMAII.
 * Ele roda somente no backend e é enviado ao Gemini como "systemInstruction".
 *
 * IMPORTANTE: sempre que você criar, mudar ou remover uma tela/recurso do
 * sistema, atualize o texto de BASE_DE_CONHECIMENTO abaixo. A IA só sabe o que
 * está escrito aqui — se estiver desatualizado, ela vai orientar errado.
 */
...
const ROLES_VALIDAS = Object.keys(PERFIS)

const PAPEL_E_REGRAS = `
...
/**
 * Monta a instrução de sistema enviada ao Gemini.
 * @param {string} role - perfil atual do usuário (diretor, coordenador, professor, aluno, responsavel)
 */
function buildSystemInstruction(role) {
  const perfil = PERFIS[role]
  const contexto = perfil
    ? `PERFIL ATUAL DA PESSOA: ${perfil}. Ela está usando o painel desse perfil e só enxerga as telas do menu dele.`
    : 'PERFIL ATUAL DA PESSOA: não informado.'

  return `${PAPEL_E_REGRAS}\n\n${contexto}\n\n=== BASE DE CONHECIMENTO DO EMAII ===\n${BASE_DE_CONHECIMENTO}`
}

module.exports = { ROLES_VALIDAS, buildSystemInstruction }
